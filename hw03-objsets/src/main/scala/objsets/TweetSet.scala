...
class Empty extends TweetSet {
  ...
  def getElem: Tweet = throw new java.util.NoSuchElementException("Empty Set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    if (this.remove(this.elem).isEmpty) {
      new Cons(elem, Nil)
    } else {
      val hi = this.mostRetweeted
      new Cons(hi, (this.remove(hi)).descendingByRetweet)
    }
  }
